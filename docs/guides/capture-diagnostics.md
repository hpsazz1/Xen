...
预测阶段提供 `PredictionStrength`、`PredictionVelocityX/Y`、`PredictionAccelerationX/Y`、`PredictionOffsetX/Y`、`ViewMotionX/Y`、`PredictionDirectionLocked` 和 `PredictedX/Y`。速度使用补偿自身鼠标输出后的短窗稳健回归结果；加速度仅供噪声诊断，不参与控制输出。分析器输出预测生效比例、提前距离P50/P95及预测侧翻转次数。目标框仅作原始检测审计，不参与提前量计算。无目标、跟踪器滑行帧、目标ID切换或瞄准状态切换会立即清空预测状态。新模型测试必须核对 `ControllerRevision=8`。
