...
分析器使用 `FinalMx/My` 计算实际输出counts/s，并从请求像素与请求计数恢复counts/px，输出 `ApproxClosedLoopLagMs`。只有文件名包含 `reverse` 或 `reversal` 的往返场景才统计持续误差侧翻转和±8 px中心恢复，单向场景固定不报告反转。默认反转误差侧阈值为12 px，可用 `ReversalErrorThresholdPx` 覆盖；默认剔除不足500 ms或预热后少于30个有效样本的碎片，可用 `MinTrialDurationMs/MinTrialSamples` 调整。首轮数据结论与PI候选见 `docs/017移动目标首轮分析与PI候选20260713.md`，DML积分解卷绕和高频反转见 `docs/019DML移动复测与积分解卷绕20260714.md`、`docs/020持续移动与高频反转优化20260714.md`。
