...
分析器使用 `FinalMx/My` 计算实际输出counts/s，并从请求像素与请求计数恢复counts/px，输出 `ApproxClosedLoopLagMs`。反转不再由相对观测速度换符号直接判定，而是使用持续误差侧翻转和±8 px中心恢复；首轮数据结论与PI候选见 `docs/017移动目标首轮分析与PI候选20260713.md`。
