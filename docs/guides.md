...
逐帧明细默认只输出 `domain_fov106_2560x1440_94fps_20ms_1x`。定位汇总中的特定失败域时追加 `--cross-domain-detail-variant <cross_domain_summary.csv中的精确Variant名称>`；名称不属于162项必测矩阵时程序直接失败，避免误以为空trace代表无问题。`cross_domain_decision.txt`会记录实际选择，多个场景追加到同一CSV时只写一个表头。
