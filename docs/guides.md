...
| [手动控制权仲裁实现](199手动控制权仲裁实现20260721.md) | 了解按住自动锁定时的人手优先输入融合、Raw Input自身回注过滤和恢复迟滞。 |
...
| [机器Profile响应覆盖阻断与解耦](175机器Profile响应覆盖阻断与解耦20260720.md) | 查看L3/L2九文件实机A/B、static硬失败及主动时序证据与运行应用的安全解耦。 |
| [机器Profile最终验收与Active就绪审计](176机器Profile最终验收与Active就绪审计20260720.md) | 查看修复后Level 3九文件PASS、P1闭环结论及active仍受P0-5阻断的完整调用链依据。 |
| [P0-5已提交端点守卫重评](179P0-5已提交端点守卫重评20260720.md) | 在统一保留迟到命令的物理契约上复验端点守卫，查看四端点公共域477→485、零损失及继续HOLD_SHADOW的边界。 |
| [P0-5检测恢复动态水平相位诊断](180P0-5检测恢复动态水平相位诊断20260720.md) | 用四端点逐帧证据区分恢复后的估计器冷启动、物理终点残差和连续限速相位。 |
| [P0-5实机重捕获包络采集协议](181P0-5实机重捕获包络采集协议20260720.md) | 规定left/right实机完整reset重捕获采集、身份校验、分析命令和进入离线候选评估的门禁。 |
| [P0-5实机重捕获包络复核](182P0-5实机重捕获包络复核20260720.md) | 保留固定20帧口径的首次实机复核过程；其退出率结论已由183号实际退出帧重评取代。 |
| [P0-5实机重捕获实际退出帧重评](183P0-5实机重捕获实际退出帧重评20260720.md) | 将20帧改为最低观测长度，按同一reset代次扫描真实退出帧并确认原34个代表事件全部退出。 |
| [P0-5恢复窗口限域速度反事实](184P0-5恢复窗口限域速度反事实20260720.md) | 在冻结检测队列上验证1800 counts/s仅限恢复窗口的离线反事实，并确认四端点共同域零损失。 |
| [P0-5恢复速度建议影子诊断](185P0-5恢复速度建议影子诊断20260720.md) | 查看仅写CSV的1800建议裁剪、正式1440退出锁定、静态预算和实机采集门槛。 |
| [P0-5恢复速度建议首轮实机复核](186P0-5恢复速度建议首轮实机复核20260720.md) | 查看r65首轮left/right恢复建议、自然退出/中断分类、CSV舍入修正和left补采门槛。 |
| [P0-5恢复建议静态预算与路径上限](187P0-5恢复建议静态预算与路径上限20260720.md) | 查看首帧静态预算为何不能预测真实退出，以及冻结1440路径上的1800建议为何只能作为非因果上界。 |
...
跨域回放在检测短失时必须重置控制状态，但不得撤销已经提交给物理相机的命令；legacy、candidate、整步响应和有限响应必须遵守同一生命周期。纠正后的14/0、14/2、15/0、15/2基线为479/480/479/479，共同交集477/810。历史524/480/524/479只用于复现缺陷，不得再作为P0-5晋级基线。

已提交端点守卫可用`--candidate-committed-endpoint-guard 1`离线复现。它按物理命令中心加半响应宽度估算已发送命令的最终位置，只裁剪与当前终点残差同向且超出半个count取整余量的请求；默认关闭，轨迹整形启用时强制失效，不进入正式配置、UI、shadow或active。统一契约下四端点结果为485/488/485/485，共同交集485/810且原477项零损失，但收益全部来自static，动态left/right阻断未解除。
