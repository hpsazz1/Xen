...
P0并行重构的CSV字段和r30的`FinalMx/FinalMy`写在同一行。P0-0只贯通模式、原始观测、重置代次以及估计/控制/轨迹占位字段；P0-1继续写入完整时间轴、分段年龄和命令发送确认字段；P0-2新增`ViewMotionShadowValid`、固定延迟、X/Y角度比例、实测视线角、观测/控制时刻相机角、稳定视线角和相对误差字段；P0-3新增角度、相对视线角速度、协方差、innovation、innovation variance、NIS和前馈可信度字段。P0-2/P0-3只读取完整源HFOV/VFOV、设备实际成功发送的counts和检测置信度，不生成设备输出。详细边界见`docs/069P0-0影子链路骨架实现20260715.md`、`docs/070P0-1完整时间轴贯通20260715.md`、`docs/071P0-2角度坐标与延迟相机响应20260715.md`与`docs/072P0-3相对视线Kalman影子链20260715.md`。
