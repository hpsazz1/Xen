...
| `move_integral_time_ms` | `320` | 五类NDI移动目标标准PI积分时间，单位毫秒；0为关闭，非零最小50 ms。用于消除匀速目标的比例稳态误差；有效积分输出在进入中心区及轻微越心时继续保持，反向误差达到稳定半径后清除对应轴旧积分。 |
| `aim_pipeline_mode` | `legacy` | 新旧链路迁移模式。`legacy`只运行r31；`shadow`在同一帧记录新链路独立状态但不向设备队列写命令；P0-0至P0-5请求`active`会保留请求值并安全降级为`shadow`。 |
...
| `aim_shadow_vertical_catch_up_deg` | `0.8` | P0-5高速横移的垂直追赶角误差阈值，范围0~20°；0关闭。仅水平相对LOS速率至少5°/s且垂直误差达到水平误差15%时生效。 |
...
P0并行重构的CSV字段和r30的`FinalMx/FinalMy`写在同一行。P0-0只贯通模式、原始观测、重置代次以及估计/控制/轨迹占位字段；P0-1继续写入完整时间轴、分段年龄和命令发送确认字段；P0-2新增角度坐标与固定延迟相机响应；P0-3新增Kalman状态与可信度；P0-4A新增控制分解；P0-4B新增请求/输出序号、整形模式、固定tick、目标与实际速度、加速度、jerk、双精度位移、整数余量、整形延迟和跳过tick。所有新字段仍只用于shadow，同帧整形整数counts不会进入设备队列。详细边界见`docs/069P0-0影子链路骨架实现20260715.md`至`docs/074P0-4B确定性轨迹执行层20260715.md`。

P0-4A控件位于UI“瞄准 → 移动参数 → 新控制器影子参数”。安全晋级顺序固定为P反馈、速度前馈、角度积分、经验提前；默认配置只启用P反馈，其余三项为0。

P0-4B控件位于UI“瞄准 → 移动参数 → 确定性轨迹影子参数”。默认`off`用于证明接入等价；切换`trapezoid`只改变shadow候选。固定输出线程接管设备和latest-only真实队列属于P0-6，当前UI已明确不发送命令。
...
以下WindMouse配置属于历史兼容字段。当前r30活动链路与对应UI均未启用这些算法，修改这些值不会改变当前基础移动输出。后续不会直接恢复随机Wind/Bezier/EMA链路，而是按`docs/068基础移动与预测重构复核计划20260714.md`设计确定性的速度、加速度和jerk约束轨迹整形层；该层用于运动连续性、设备约束和可重复测试，不用于规避检测或模仿人类身份。

| 键 | 默认值 | 说明 |
|---|---:|---|
| `wind_mouse_enabled` | `false` | 历史WindMouse开关；当前活动链路不读取。 |
