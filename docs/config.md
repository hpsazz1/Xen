...
P0并行重构的CSV字段和r30的`FinalMx/FinalMy`写在同一行。P0-0只贯通模式、原始观测、重置代次以及估计/控制/轨迹占位字段；P0-1继续写入完整时间轴、分段年龄和命令发送确认字段；P0-2新增角度坐标与固定延迟相机响应；P0-3新增Kalman角度、相对视线角速度、协方差、innovation、NIS和前馈可信度；P0-4A新增控制有效性、二维限速、积分冻结、P/前馈/提前/积分分解、限速前后counts和周期预算。所有新字段仍只用于shadow，同帧`AimPipelineRequestedCountsX/Y`不会进入设备队列。详细边界见`docs/069P0-0影子链路骨架实现20260715.md`至`docs/073P0-4A角度控制器拆分20260715.md`。
