...
P0并行重构的CSV字段以`AimPipeline`开头，和r30的`FinalMx/FinalMy`写在同一行。P0-0只贯通模式、原始观测、重置代次以及估计/控制/轨迹占位字段，不生成新的角度估计或设备输出；P0-1继续写入`BackendReceiveNs`、`CaptureSubmitNs`、`InferenceStartNs`、`InferencePublishNs`、`ControlTimeNs`、分段年龄和命令发送确认字段。详细边界见`docs/069P0-0影子链路骨架实现20260715.md`与`docs/070P0-1完整时间轴贯通20260715.md`。
