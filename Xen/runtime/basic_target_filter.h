...
public:
    struct Result
    {
        ...
        bool motionTrendActive = false; // 当前帧是否使用趋势速度控制滤波响应
                  ...
                  double maxObservationGapSeconds = 0.10)
    {
        if (!std::isfinite(x) || !std::isfinite(y))
            return { filteredX_, filteredY_, 0.0, 0.0, 0.0,
                     std::hypot(trendVelocityX_, trendVelocityY_),
                     useMotionTrend, 0.0 };
        ...
        if (!initialized_)
        {
            ...
            return { x, y, 0.0, 0.0, 0.0, 0.0, useMotionTrend, 0.0 };
        }
        ...
        if (rawDistance > std::max(24.0, screenWidth * 0.20))
        {
            ...
            return { x, y, observedVelocityX, observedVelocityY,
                     observedSpeed, 0.0, useMotionTrend, 0.0 };
        }
        ...
        const double motionRatio = std::clamp(
            (useMotionTrend ? motionTrendSpeed : observedSpeed) /
            std::max(120.0, screenWidth * 1.25), 0.0, 1.0);
        ...
        return {
            ...
            observedSpeed, motionTrendSpeed, useMotionTrend, residual
        };
    }
