...
<h1>开火采样调试报告</h1><p class="note">命令 ACK 代理模型 · 初始静止未经实测确认 · 采样次数不等于子弹数<br>“模型阈值内”只表示当前参数下的估计。游戏中是否停稳，以人工观察独立记录。</p>
...
$('status').textContent=a.execution_success===true?'动作执行完成':`动作未完整完成：${a.execution_failure||'状态未知'}`;
...
const ctx=$('plot').getContext('2d'),points=[s.down_model,...(s.samples||[])].filter(m=>m?.valid&&typeof m.speed_ratio==='number');ctx.clearRect(0,0,1000,240);const maxX=Math.max(1,...points.map(p=>(p.time_ns-s.down_ack_ns)/1e6)),maxY=Math.max(1.5,...points.map(p=>p.speed_ratio))*1.1;const x=t=>50+t/maxX*900,y=v=>205-v/maxY*180;
ctx.font='14px sans-serif';ctx.fillStyle='#a9bfd0';ctx.fillText('0 ms',40,230);ctx.fillText(`${maxX.toFixed(1)} ms`,890,230);ctx.fillText('1',15,y(1)+5);ctx.strokeStyle='#f3c67a';ctx.setLineDash([6,5]);ctx.beginPath();ctx.moveTo(45,y(1));ctx.lineTo(960,y(1));ctx.stroke();ctx.setLineDash([]);ctx.strokeStyle='#61c6db';ctx.beginPath();points.forEach((p,i)=>{const px=x((p.time_ns-s.down_ack_ns)/1e6),py=y(p.speed_ratio);i?ctx.lineTo(px,py):ctx.moveTo(px,py)});ctx.stroke();points.forEach(p=>{ctx.fillStyle=p.speed_ratio<=1?'#7dddaf':'#ffcb88';ctx.beginPath();ctx.arc(x((p.time_ns-s.down_ack_ns)/1e6),y(p.speed_ratio),4,0,Math.PI*2);ctx.fill()})}
