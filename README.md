...
释放清理成功且同方向键盘报告持续可信时，只撤销旧制动资格，保留真实输入连续性；需要重新武装时仍须松开再按允许键，
不额外要求WASD全松。GSI许可与键盘流分别校验，武器资料恢复不会自动解除重新武装要求。
...
默认切枪键1–5/Q归还键盘后，松开切枪键即可重新核验，无需松开允许键。取消、失焦、输入异常、身份改变、自身死亡或GSI过期仍要求健康松键重新武装。异常LEFT UP清理不启动下一轮，
