...
HUD急停面板显示最近偏差、平均快慢、优秀率、波动、最快/最慢和整体习惯，折线按完美青、优秀绿、偏早黄、偏晚红分类。
...
计划先校验并准备，参数冻结在独立目录；真实测试须由用户勾选本次物理输出、输入
`AUTO_STOP_COUNTERPULSE` 并点击启动。既有CLI仍要求 `-AllowPhysicalOutput` 与确认令牌。
测试与生产Runtime互斥，复用已有设备连接；实际命令超时需不超过100ms、连接超时不超过2000ms，
超过时在设置调整并保存后重新准备。运行、录制或清理未结束时不能切换设备；结束不自动恢复Runtime。
离线重评、默认基准及动作候选派生不连接设备。旧Run只读，每次重评和复测产生新目录。

独立射击每组固定15次左键按住，仅有按住时长和DOWN提交间隔两个参数，次数不是子弹数。
可从辅助页管理的共享武器目录带入草稿；测试范围与生产表范围分别校验，不自动回写生产资料。
急停测试与射击测试保留各自原有调度；提交间隔、DOWN ACK到UP提交、ACK到ACK分别显示，缺测保持未知。
原地射击不启用模型HUD，不生成后座恢复或真实停稳判断。

切页和最小化不结束任务，隐藏HUD只影响显示，关闭HUD请求停止。关闭主窗会先取消并等待后台清理；
...
自动急停开启后，按住快捷键且准星进入配置人物的完整检测范围时独立制动，不要求开启自动扳机。
...
自动急停可显式选择“使用H40测试时序”：全零确认、按实际方向反向按下并从协议ACK保持40ms，
反向释放协议ACK后等待18ms，再提供估计完成资格。正常操作不注入测试中的100ms自动移动，
也不叠加测试循环的200ms射后等待；后续点射由武器间隔统一约束。原指数策略保留供回退，
旧配置不自动切换。H40时序迁移不等于不同移动时长或方向的实机组合已经通过。
