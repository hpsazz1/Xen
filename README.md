...
运行切换、按住启用和急停都使用可保存的 Win32 虚拟键集合，支持键盘及鼠标左/右/中/侧键；
同一功能绑定多个键时，按任意一个都生效，跨功能不得复用同一键。运行切换默认只绑定一次 F8，
按当前 Runtime 状态执行“F8 启动 / F8 停止”；按住启用对绑定集合取 OR，急停和运行切换按每个
物理键的独立上升沿触发，持续按住不会重复提交。配置页点击对应绑定后直接按键追加，按 Esc
清空该功能全部绑定；空集合表示禁用，物理输出开启时按住启用和急停集合仍不得为空。
...
│   ├── keyboard/              # 多键/鼠标绑定、运行切换、按住启用与急停轮询
