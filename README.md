...
- Overlay 采用 152 px 居中标签栏和带 12 px 外边距的独立工作区。自绘标题栏在侧栏交界线处分色：左段与侧栏同色，右段与工作区同色，并随浅色/深色主题同步切换；侧栏仅保留概览、检测、瞄准、输入和设置五个标签。内容面板使用 Codex 任务总结式细边框卡片，配置页保存操作固定在页头，运行期间锁定需重建资源的配置。
