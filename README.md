...
场景化实机验收统一使用
`build-matrix-final-cpu/Release/models/14wv11.onnx`。禁止人工复制旧配置后修改参数；
...
绝对脚本路径，因此不依赖 PowerShell 当前目录。观察任务固定用 F8 切换 Runtime：停止态按一次
启动，运行态再按一次停止，无需切回 Xen 窗口。固定模型元数据为 `C0=ct_body`、`C1=ct_head`、
`C2=t_body`、`C3=t_head`；配置同时纳入 CT/T 的身体与头部类别。单个目标只属于 CT
或 T，自动报告按原始类别分别导出数量和置信度，不把另一阵营未出现视为失败。该模型实际输入为
`320x320`，因此实机验收固定使用主机中心 `320x320` ROI。
示例：
...
OR，物理输出急停和运行管线启停按每个物理键的独立上升沿触发，持续按住不会重复提交。配置页
点击对应绑定后直接按键追加，按 Esc
清空该功能全部绑定；空集合表示禁用，物理输出开启时瞄准输出和物理输出急停集合仍不得为空。
控制台五页导航、全局动作和全部配置项均提供悬浮说明；配置项标签后的 `?` 是统一说明入口，
禁用状态下仍可查看其生效条件。物理输出急停只锁存输出安全门，不停止截图和检测，释放急停键
后还必须在界面中手动复位。

KMBOX NET 实现 Runtime 相对移动和只读物理键鼠 monitor，不包含按键屏蔽、LCD 或自动轨迹
接口。当前实验室设备 IPv4、端口和屏幕显示的 8 位十六进制 UUID 已固化为代码默认值，但不会
因此开启物理输出；切换设备时必须在配置页显式更新。后端使用 16 字节 connect 和 72 字节 move 小端序数据报，并严格校验 ACK 的来源
...
│   ├── keyboard/              # 多键/鼠标绑定、运行管线启停、瞄准输出与物理输出急停轮询
...
- Aim 对头身观测归并和高/低置信轨迹关联均执行全局一对一分配，轨迹状态按真实帧间隔使用
  轻量 alpha-beta 常速度更新；当前锁定通过优势、连续确认和冷却保持，且在重叠的已确认重复轨迹
  争夺同一观测时只获得软关联优先权，既避免短暂 LOST 后切 ID 的硬零顿挫，也不劫持明显分离的
  真实目标。多目标交叉时不会按检测下标直接换目标。身体框作为头身模型的归一化坐标系，头框
  连续缺失期间沿用稳定框内基础瞄点，
  不在头部中心和身体默认点间上下切换。预测点只按本机 `captured_at` 到控制时刻的可测年龄提前，
  允许越过当前目标框，但提前向量受用户设置的目标框对角线比例硬门禁；无论预测是否开启，基础
  追踪点始终锁在目标框内，只有应用提前量后的最终预测点允许出框。控制方向不得背离当前基础点
  或预测点，单帧步长不得越过该点，随后执行二维
  counts 向量限幅和量化残余累计。Overlay 的基础移动只保留目标置信度、搜索范围、身体瞄准高度、
  移动强度和最大步长；关联、切换和平滑细节由内部自动处理。搜索范围只限制目标获取、挑战者
  切换和鼠标命令，按住瞄准键后动态收缩，但所有 Detection 始终进入轨迹更新，圈外已锁定轨迹
  保持身份并仅暂停命令。当前人工基线默认开启预测提前，只额外显示最大提前距离；进入/退出
  双阈值会在目标越过准星、反向或回到收敛区时先撤销提前；基础点沿原预测方向连续归位后才在
  后台重新武装，重新离开还需连续观测确认。进入距离只看运动方向上的轴向误差，正交基础误差
  不会误触发前探，从而避免反向回拉和重新预测形成震荡。长时间完全重合或遮挡仍需要身份真值
  触发 ReID/更高阶模型评估，当前实现不伪造这类确定性。prediction 关闭且延迟补偿开启时，
  tracking X 通过 CUSUM/共同平移或位置门后先进入有界反馈探针；首窗仍走平滑与 slew，
  但命令封顶 3 counts。仅由位置面积启动的探针在第二窗最多保留 1 count，并且只有当前
  CUSUM/共同平移事实确认后才升级为全量换向；两窗到期仍无独立确认则取消并从新鲜位置面积
  重试，不能仅凭等待时间提交方向。若快速 CUSUM 恰在旧方向 `delayed_command` 开始生效的
  帧越门，同样只启动有界探针，并清除该批旧响应证据，必须由后续新鲜几何再次确认。
  tracking X 还会在旧延迟命令已产生可信闭合响应时禁止继续增幅；该闭合事实每持续满 1.5 个已配置
  的控制反馈窗，才额外卸载最多 1 count，直至物理最小输出，减少越心前的旧方向存量。该节拍不
  读取人物绝对速度或游戏档位，也不改变 Y、prediction、配置增益和真实反向的有界探针契约；
  离线闭环回归只验证安全边界，真实 KMBOX 观感仍以对应人工 Run 为准。
...
- 检测页提供显式启用的实时 ROI 诊断。Runtime 最多 10 FPS 发布最长边 512 的同帧 BGRA、
  Detection 和 Aim 快照，三个预分配槽只保留最新帧；该频率只限制诊断抽样，不限制 Detector、
  Aim 或控制管线。页面预览使用复用的 D3D11 动态纹理；“置顶”开关另建不抢焦点的 Win32
  TOPMOST 窗口，按新预览序号通过复用 GDI 内存位图一次性绘制画面、检测框、当前目标、瞄点和
  状态，切换主控制台标签页后仍持续显示。停止、故障或关闭订阅会清除旧画面；两个视图均关闭时
  不执行 resize、颜色转换或图像复制。GPU 互操作模式仍禁止 CPU 预览回读；普通置顶窗口不保证
  覆盖绕过 DWM 的独占全屏。`XenBenchmark.exe` 不创建 Overlay，因此正式基准不包含该支路。
...
2026-08-04 的真实双机 `320x320` 验收已覆盖静态档位、重复左到右横移和五分钟长稳。统一使用
`14wv11.onnx`，辅机物理输出禁用；全部正式轮次推理失败、源端/传输丢帧和非法包均为零。按用途
选择链路，不把配置 FPS 当成实测 FPS：

| 用途 | 推荐配置 | 真实动态/长稳结论 |
|---|---|---|
| 默认稳妥 | `XUDP 120 FPS + DirectML` | 动态 7,189、长稳 35,874 个成功样本，四类丢弃/覆盖全零；长稳 total P95/P99 `4.126/4.697 ms` |
| 已有 OBS 工作流 | `NDI 120 FPS + DirectML` | 动态仅 1 次 Runtime 覆盖，长稳 36,002 样本四类全零；长稳 total P95/P99 `4.125/4.592 ms` |
| 最低辅机处理延迟 | `NDI 240 FPS + TensorRT` | 动态/长稳维持约 240 FPS，total P95/P99 `2.201/3.006` 与 `2.170/2.889 ms`；最终累计覆盖 `47/17`，正式窗口只缺 `1/2` 帧（`0.0069%/0.0028%`），仍不满足严格零覆盖 |
| 协议完整性性能档 | `XUDP 240 FPS + DirectML` | 动态/长稳四类全零，total P95/P99 `3.072/3.512` 与 `3.078/3.533 ms`；当前 CS2 + OBS 并行负载下实际仅约 `200~205 FPS` |

XUDP 提供帧号、分片、主机几何和 JPEG SHA-256，严格生产默认选 120 档；240 档只能按实测约
`200 FPS` 使用，不能宣称稳定 240 FPS。NDI 不需要 `XenSender`，已在 OBS 中完成采集时部署更简单；
120 档约占主机专线 `20.8 Mbps`。TensorRT 240 档延迟最低，但仅适合明确接受少量最新帧覆盖的
场景。裸 UDP MJPEG 缺源帧号和完整性语义，且既有 60 FPS P95/P99 明显更高，只保留兼容或排障。
详细证据见本地文档 `docs/050_双机采集链路性能对照_20260804.md`。

本项目后续双机测试开发已明确采用 `NDI 240 FPS + TensorRT` 作为实验室基线：精确源名保持
`HPSAZZ (Xen-ROI-320)`，输入为 `320x320`，TensorRT 启用 FP16、CUDA Graph 和 GPU 前处理，
物理输出默认禁用。该选择接受已观测到的低比例 Runtime 最新帧覆盖，不改变上表面向一般使用的
稳妥推荐；后续每份报告仍必须单列覆盖数量和比例，并要求推理失败、源端/传输丢帧及非法包为零。
...
基准和使用建议；当前代码默认值已固化为本实验室的 NDI + TensorRT 人工基线。结果不能外推到其他模型、Intel GPU/NPU 或双机链路；
