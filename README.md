...
调试页现提供固定靶点的“准备画面标定 / 准备采集新曲线 / 准备测试曲线”流程。
先停止Runtime，保持既有KMBOX连接，在调试页启用并保存一个不冲突的测试键（例如未占用的F6）。
填写实际游戏灵敏度、选择武器；首次先准备画面标定，回游戏对准静止且有纹理的靶面，按测试键后松开。
标定只执行有限的两轴位移，不开枪；随后准备采集或测试，再按同一键执行一组自动扫射。
每按一次只执行一组，忙碌不排队；紧急停止键、失焦、观测失配和超限均停止。常规Aim/扳机不同时运行。

没有曲线也能采集：先测量画面位移与设备counts的映射，再从固定背景运动生成视角补偿候选。
它不把准星动画或GSI弹药包当作真实逐发弹着；保存的候选仍未校准。点击查看本组画面、确认固定靶点和画面可用后，
可保存候选用于下一次测试。采集图像等比限制至960×540、最多约30帧/秒，保留实际源/接收时间与几何，不伪造固定采样时序。
画面配置、实际尺寸或灵敏度变化后重新标定。背景接近观测范围边界或失配的数据不参与训练。

“本阶段目标发数”和“每阶段追加发数”可按武器调整并保存，后坐力大的武器可从1至2发开始。
目标发数由GSI弹药变化辅助停止，并有最长扫射时限；GSI延迟可能导致超发，未知或不符目标的组只保留原始记录。
同一曲线累积三组训练和两组独立验证画面后，点击优化，程序另存候选并载入供复测，禁止重复消费验证数据。
