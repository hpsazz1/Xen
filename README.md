...
- Aim 对头身观测归并和高/低置信轨迹关联均执行全局一对一分配，轨迹状态按真实帧间隔使用
  ...
  tracking X 还会在旧延迟命令已产生可信闭合响应时禁止继续增幅；若反向门仍阻塞、两条框边
  随后重新明确支持已确认方向，则只在既有归一化位置门和反馈窗共同限定的一次性预算内桥接
  1 count，减少硬零停顿而不改变 Y、prediction、配置增益或真实反向的有界探针契约。该路径
  已通过离线闭环回归，真实 KMBOX 观感仍以对应人工 Run 为准。
