...
急停“跨轮起步间隔 / ms”（`restart_interval_ms`）配置上次反向释放ACK到下一轮正向按下提交的最小间隔，范围0～2000ms，0保持原行为。与左键完整释放和原有起步目标取较晚者，不截短武器按住；首次移动无前次反向释放，不应用该间隔。动态模式下增加间隔会压缩移动余量，耗尽时停止并清理。仅反向制动模式可设置非零值，切换其他基准动作会清零。
