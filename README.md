...
- Desktop Duplication 收到 `DXGI_ERROR_ACCESS_LOST` 且 D3D11 设备仍有效时，会在 1 秒
  ...
  超时均仍进入 FAILED。成功重建返回 `NO_FRAME`，不发布旧帧，次数在 schema 11
...
- Aim 对头身观测归并和高/低置信轨迹关联均执行全局一对一分配，轨迹状态按真实帧间隔使用
  ...
  tracking X 通过 CUSUM/位置门后先进入一个反馈确认窗；窗内仍走平滑与 slew，
  但命令封顶 3 counts，首个响应窗结束后候选仍成立才确认新方向。
...
schema 11 正式入口会逐帧校验 Aim：基础追踪点只要离开当前目标框即失败；预测提前点允许离开
...
开关和最大提前距离两个 Aim 参数。`<prefix>.network.json` 会带 schema 11 Aim 汇总，最后发布
才表示网络接收报告完整。
...
清单，并在应用退出后收集本轮新增 Runtime CSV/JSON、日志和 Aim schema 11 汇总。静止、左移、
