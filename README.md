...
2026-08-04 的真实双机 `320x320` 静态 A/B 已覆盖 60/120/240 FPS。当前默认测试档建议使用
`NDI 120 FPS + DirectML`：实际 119.999 FPS、7,200 个成功样本、四类丢弃/覆盖均为零，total
P95/P99 为 `4.091/4.235 ms`。最低延迟候选为 `NDI 240 FPS + TensorRT`：实际 240.068 FPS、
total P95/P99 为 `2.002/2.247 ms`，但 14,402 个样本中 Runtime 主动覆盖 13 帧，约 `0.09%`。
`NDI 240 FPS + DirectML` 两轮只覆盖 1/2 帧，total P95/P99 约 `2.29/2.50 ms`，仍不属于严格
零覆盖档。240 FPS 已纳入后续测试，但只有动态与五分钟长稳复测通过后才考虑改为默认。

XUDP Sender 已在提交 `6f0732c` 修复相对截止时间造成的发送节拍漂移。相同双机 GeometryStatic
复测中，120/240 请求档的主机实际发送率为 `119.999/239.765 FPS`，辅机 DirectML Capture 为
`119.591/239.943 FPS`；两档均为零发送/推理失败、零源端丢帧、零传输丢帧、零非法包和零
Runtime 覆盖，total P95/P99 分别为 `4.149/4.274 ms` 与 `2.425/2.556 ms`。XUDP 因此已成为
兼顾高刷新和协议完整性的生产候选；NDI 仍保留部署成熟、带宽可控的默认对照，裸 UDP 仅作兼容。
动态往复和五分钟长稳仍需完成后才能改变最终默认链路。详细证据见本地文档
`docs/050_双机采集链路性能对照_20260804.md`。
