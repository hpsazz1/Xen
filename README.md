...
自动扳机的“允许开枪”可关闭，保留检测、资格判断与急停联动调试，但不发送自动左键按下，
...
另可显式开启“使用估计完成联动”：复用独立急停四键接管持续有效的估计完成状态，与武器冷却并行，
...
再按下。普通心跳不会反复取消。未启用共享武器资料时使用全局扳机参数。
...
后坐力页面的“共享武器点射节奏”独立管理33种武器的按住时长和按下间隔，关闭压枪也能使用。
开启后可由GSI选择武器，或明确手选；手选与GSI同时启用时必须与实际上下文匹配。未知/禁用资料不沿用上一把。
默认沙鹰60/600ms、AK与M4为60/300ms；R8仅留档，随机波动暂未启用。仅点射使用该表，连续按住不套用点射间隔，
也不缩放压枪弹道。点射按住从DOWN协议ACK计时，间隔从DOWN提交计时；UP未确认不发送下一DOWN，调度迟到不补发。
...
压枪默认关闭，默认采用 GSI 自动识别武器并精确匹配已校准弹道。GSI 配置在设置页：
