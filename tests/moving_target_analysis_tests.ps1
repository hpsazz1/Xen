...
'@ | Set-Content -LiteralPath $csvPath -Encoding UTF8
    ...
    Assert-Equal 1 $trials[0].ReversalCount 'Persistent tracking-error side reversal must be counted.'
    ...
    Assert-Equal 1.344 $trials[0].EstimatedCountsPerPixel 'Counts-per-pixel must be derived from exported requests.'
    ...
    Assert-Equal 3 $rerunMetrics.Count 'Repeated analysis must ignore its own root-level summary CSV.'
}
finally {
