...
Assert-True ($networkReceiverText -match
    ...
    $networkReceiverText -match
        'if \(\[int\]\$report\.schema -notin @\(17, 18, 19\)\)' -and
    $networkReceiverText -match 'Assert-XenAimBackgroundReportFields') `
    "网络接收脚本必须把 Aim 参数和 schema 17/18/19 汇总传入正式报告。"
...
Assert-True ($runtimeBenchmarkText -match
    ...
    "Runtime 正式入口必须消费 schema 17/18/19，并把预测出框作为观测而非违规。"
...
Assert-True ($detectorVideoBenchmarkText -match
        'if \(\[int\]\$runtimeReport\.schema -notin @\(17, 18, 19\)' -and
    ...
    "Detector 视频回放必须接受 schema 17/18/19，并按实际报告保留版本。"
