...
Assert-True ($networkReceiverText -match
    ...
    $networkReceiverText -match
        'if \(\[int\]\$report\.schema -notin @\(17, 18, 19, 20\)\)' -and
    $networkReceiverText -match 'Assert-XenAimBackgroundReportFields') `
    "网络接收脚本必须把 Aim 参数和 schema 17/18/19/20 汇总传入正式报告。"
...
Assert-True ($runtimeBenchmarkText -match
    'if \(\[int\]\$report\.schema -notin @\(17, 18, 19, 20\)\)' -and
    $runtimeBenchmarkText -match 'Assert-XenAimBackgroundReportFields' -and
    ...
    "Runtime 正式入口必须消费 schema 17/18/19/20，并把预测出框作为观测而非违规。"
...
Assert-True ($detectorVideoBenchmarkText -match
        'if \(\[int\]\$runtimeReport\.schema -notin @\(17, 18, 19, 20\)' -and
    $detectorVideoBenchmarkText -match 'Assert-XenAimBackgroundReportFields' -and
    ...
    "Detector 视频回放必须接受 schema 17/18/19/20，并按实际报告保留版本。"

$detectorModelTestText = [System.IO.File]::ReadAllText(
    (Join-Path $RepositoryRoot "tests/detector_model_test.cpp"))
Assert-True ($detectorModelTestText -match
        'sample\.aim_landmark\s*=\s*aim_landmark::inspect_head_landmark') `
    "生产 Detector-Aim 视频回放 seam 必须写入同帧 diagnostic landmark。"
...
Assert-True ($aimManualText -match
    ...
    $aimManualText -match
        '\$reportSchema\s*-notin\s*@\(13, 14, 15, 16, 17, 18, 19, 20\)' -and
    $aimManualText -match 'Assert-XenAimBackgroundReportFields' -and
    $aimManualText -match '(?m)^\s*schema\s*=\s*2\s*$') `
    "Aim 人工入口必须兼容 schema 13 至 20，并把控制诊断和锁定帧计数写入自动汇总。"
